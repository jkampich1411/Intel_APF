...
export default class APF_TcpForward_Request extends Message {
  override type = APFMessage.TCPFORWARD_REQUEST;
  ...
  constructor(
    ...
    if(!header)
      this.header = new APF_Global_Request("tcpip-forward", true);
  ...
  static fromBytes(data: Buffer): [APF_TcpForward_Request, number] | null {
    let lenCurr = 8;
