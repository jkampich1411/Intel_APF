...
export default class APF_Keepalive_Options_Reply extends Message {
  override type = APFMessage.KEEPALIVE_OPTIONS_REPLY;
  ...
  static fromBytes(data: Buffer): [APF_Keepalive_Options_Reply, number] | null {
    if(data.length < 9)
      return null;
    ...
    return [new APF_Keepalive_Options_Reply(keepaliveInterval, readTimeout), pos];
  }
}
