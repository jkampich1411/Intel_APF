...
export default class APF_Keepalive_Request extends Message {
  override type = APFMessage.KEEPALIVE_REQUEST;
  ...
  static fromBytes(data: Buffer): [APF_Keepalive_Request, number] | null {
    if(data.length < 5)
      return null;
    ...
    return [new APF_Keepalive_Request(cookie), pos];
  }
}
